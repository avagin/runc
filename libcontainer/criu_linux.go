...
func (c *Container) criuNotifications(resp *criurpc.CriuResp, process *Process, cmd *exec.Cmd, opts *CriuOpts, fds []string, oob []byte) error {
	...
	case "post-restore":
		...
		if !c.config.Namespaces.Contains(configs.NEWTIME) && configs.IsNamespaceSupported(configs.NEWTIME) {
			c.config.Namespaces = append(c.config.Namespaces, configs.Namespace{Type: configs.NEWTIME})
		}
